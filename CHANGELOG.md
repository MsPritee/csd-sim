...
### [Added] - 2026-09-17 20:45
- **Component**: Circuit Designer — Default 2-Input Gates, Body-Hugging Selection Corners & Input-Pin Redesign
- **Description**: Continued the Circuit Designer visual overhaul with four changes. (1) Every gate with a configurable input count (`INPUTS_ATTR` on AND/OR/NAND/NOR/XOR/XNOR/ODD_PARITY/EVEN_PARITY) now defaults to **2 inputs** (was 5; range stays 2–32, unary gates stay 1, controlled components stay 2). (2) Selection corners are smaller — padding 6→2, corner length 12→10 — and now hug the component's visible body instead of the full 140×100 box: gates use the new exported `gateBodyBounds(gate, inputCount)` (per-shape coordinates that grow with the input count), input pins use `INPUT_PIN_SQUARE`, all other components keep the full box. (3) The input pin was redesigned: the outer box/gray border is gone; only the inner 34×34 value square remains under the handle. Its label is now drawn **outside only** (options `top`/`bottom`/`left`/`right`, default `bottom`) via `pinLabelPosition()` — no label inside the square and no `center` option; the label **defaults to blank** (was `'A'`). Selection highlights only the inner square's edges. (4) Input pins gained a connection **facing** option `east`/`west`/`north`/`south` (default `east`); the port/dot moves to the chosen face (`inputPinPortLocal`) and `portLocalPos` for `'input'` is now facing-aware so attached wires stay in sync. Output pins are unchanged.
- **Reasoning**: Requested by the user. Most placed gates only need two inputs, so defaulting to 2 (with 2–32 still available) reduces cleanup on the canvas. The oversized dashed-corner box that exceeded the glyph looked loose, so corners were tightened to hug the actual visible shape/pin-square. The pin's outer gray box visually cluttered the row of compact pins and duplicated the inner square, so it was removed; an outside-only, default-blank label makes the value square the clear focus (as with the classic pins) while still offering four outside label directions for annotation. A facing option mirrors how real pin heads connect on different edges.
- **Impact**: Schema/attribute defaults changed — new gates now start with 2 inputs and input pins start with a blank label; existing saved projects render identically unless the new attributes are edited. Attribute-table tests for the rename keys and AND defaults were updated; the test data-testids for the pin inner square are `input-pin-square` and `pin-label`. Full suite `1880 passed / 52 failed`; all 52 failures remain confined to the pre-existing `src/tests/simulators/numbersystems/` set (no circuit-related failures; Lesson1 shows 4 vs 1 at baseline — that timing-animation file is one of the known-flaky sets, unaffected by circuit changes). `npm run build` exit 0, lint 0 errors. New tests: 7 in CircuitDesigner (default 2 inputs, `gateBodyBounds` per shape, no outer box / no default label, label options exclude `center`, label outside on left/top, facing default east + option set) and 4 in layout (`inputPinPortLocal` per facing, `pinLabelPosition` outside positions, facing-aware `portLocalPos`).
- **Files Modified**:
  - `src/core/circuit/descriptors.ts` - `INPUTS_ATTR` default 2 (comment updated), `gatePortCount` fallback 2; new `PIN_FACING` (`east`/`west`/`north`/`south`, default `east`) and `PIN_LABEL_LOC` (`top`/`bottom`/`left`/`right`, default `bottom`); input `pinDescriptor` now `[label(''), WIDTH_ATTR, PIN_LABEL_LOC, PIN_FACING, ...LABEL_STYLE_ATTRS]`, output unchanged
  - `src/simulators/circuit/layout.ts` - `INPUT_PIN_SQUARE` (34×34 at 34,28), `PinFacing`, `inputPinPortLocal(facing)` (east/west/north/south), `PinLabelLocation`, `pinLabelPosition(loc)` (all four outside the square), facing-aware `portLocalPos` input branch; removed `INPUT_DOT_X`
  - `src/simulators/circuit/GateGlyph.tsx` - exported `gateBodyBounds(gate, inputCount)` (AND2 {34,17,70,66}, NAND2 {34,17,84,66}, XOR2 {24,17,80,66}, ODD_PARITY2 {34,12,70,76}, NOT1 {54,34,56,32}); `PinSymbol` input case redesigned — inner square (`data-testid="input-pin-square"`, rx=2), facing dot, value text, width tag, and outside label via `pinLabelPosition`/`labelStyle` (`data-testid="pin-label"`); output case unchanged
  - `src/simulators/circuit/CircuitDesigner.tsx` - `SelectionCorners` pad 6→2/len 12→10; gates wrap `gateBodyBounds(...)`, input pins NO outer box + generic label suppressed and corners hug `INPUT_PIN_SQUARE`, all other components unchanged; pins receive `labelLocation`/`labelStyle`/`facing`
  - `src/tests/simulators/circuit/CircuitDesigner.test.tsx` - rename-key `pin.attrs` updated; 7 new tests (arity default 2, `gateBodyBounds`, inner-square/no-outer-box/no-default-label, label-location set, label outside on left/top, facing default + options)
  - `src/tests/simulators/circuit/layout.test.ts` - 4 new tests for `inputPinPortLocal`/`pinLabelPosition`/facing-aware `portLocalPos`
  - `src/tests/core/circuit/descriptors.test.ts` - AND inputs 5→2 in `defaultAttrs`/`portCountOf`; input `defaultAttrs`/`normalizeAttrs` updated to the 9-key set (`label:''`, `labelLocation:'bottom'`, `facing:'east'`)
  - `src/tests/stores/circuitStore.history.test.ts` - `attrs.inputs` expectation 5→2

### [Added] - 2026-09-17 19:30
- **Component**: Circuit Designer — Selection Corners, Glyph-Symbol Removal, Label Positioning & Label/Border Styling
- **Description**: Visual/glyph overhaul of the Circuit Designer. (1) Selection no longer draws a dashed rectangle around a component — only four corner brackets (`SelectionCorners`) mark the selection, keeping the shape and its wires fully visible. (2) The `&`, `≥1`, `=1`, `2k+1`/`2k` symbols are removed from the centers of all standard/numeric gates (AND, OR, XOR, XNOR, NAND, NOR, NOT, buffer and the arithmetic/comparator shapes); the plain gate shape is the only glyph now. (3) Label placement gained `left` and `right` positions (drawn OUTSIDE the gate, anchored to the horizontal centre line) and a `center` position (drawn INSIDE the shape like top/bottom); `LABEL_LOC` options are now `['top','bottom','left','right','center']` and `labelPosition(loc,w,h)` computes the anchor/pivot for each. (4) Label text styling via a small pop-up modal: color (hex, with a clear-to-default button), bold/italic/underline toggles, size (8–40 default 11) and font family (default/monospace/serif/sans-serif/cursive), with a live preview showing the current style. (5) Gate-shape border color via the same pop-up-modal pattern (hex + clear-to-default + preview), defaulting to the theme border. The style/border buttons appear only in the attributes panel (`btn-label-style` / `btn-border-color`); the underlying keys are filtered out of the normal attribute table. New schema keys: `LABEL_STYLE_ATTRS` (`labelColor`, `labelBold`, `labelItalic`, `labelUnderline`, `labelSize`, `labelFont`) and `BORDER_COLOR_ATTR` (`borderColor`), applied via `labelAware()` to every label-bearing component (dff, constant, probe, tunnel, clock, led, button, segment, adder, subtractor, comparator, negator, jk, t, sr, register, counter, ram, rom) and to all standard/numeric gates; text components get the label-style keys only (no location/border). Text components also render their label text with the chosen styling for the first time.
- **Reasoning**: The dashed selection rectangle and the center glyph symbols visually cluttered the compact gate glyphs; corner-only selection is a cleaner Logisim-consistent affordance. Left/right label options let long names live beside a gate without overlapping its body, while center labels still render within the shape like top/bottom. Verbatim label strings (colors, emphasis, size, font) and a per-gate border color are common in textbook circuit annotations, so the user asked for a small pop-up modal (instead of editing raw attribute rows) to set them — reachable only from the attributes panel to keep the canvas toolbar uncluttered.
- **Impact**: Pure presentation/`descriptors.ts` schema additions — no changes to logic engine, simulation, store state or project file format. `defaultAttrs` now include the new keys (defaults are the previous behavior: label bottom-focused, default color/size, no border override), so existing projects render identically until a new option is used. Local state persists for the session; per-item persistence is not yet added. `npm run build` exit 0, lint 0 errors in changed files; 6 new CircuitDesigner tests (corner-only selection, no center symbols, label-location options, label placed outside on `left`, label/style modal committing attrs, border-color modal with scoped `gate-body` stroke). Full suite `1873 passed / 49 failed`; the 49 failures remain exactly the pre-existing `src/tests/simulators/numbersystems/` set (DecimalToBinaryVisualizer 4, DivisionTable 2, octal Lessons 1–5 + OctalDigitCard 43) — unchanged.
- **Files Modified**:
  - `src/simulators/circuit/CircuitDesigner.tsx` - `SelectionCorners` (4 corner brackets, `data-testid="selection-corners"`) replaces the dashed selection rect; labels positioned via `labelPosition`/`labelStyleProps`; components render `borderColor`; text components apply label-style props + textColor; `AttributeTable` filters style keys into `btn-label-style`/`btn-border-color`; new `StyleModal` pop-up (label/border variants, live `setAttr` previews, `data-testid="style-modal-*"`)
  - `src/simulators/circuit/GateGlyph.tsx` - removed `symbol`/`symbolX` (no more `&`/`≥1`/`=1`/`2k±1`); added `labelLocation`/`labelStyle`/`borderColor` props; body/accent stroke = `borderColor || 'var(--border-light)'`; testids `gate-body`/`gate-label`
  - `src/simulators/circuit/layout.ts` - `LabelLocation` type, `labelPosition(loc,w,h)`, `LabelTextStyle` interface, `labelStyleProps()` (default color `var(--text-secondary)`, size 11)
  - `src/core/circuit/descriptors.ts` - `LABEL_LOC` extended to top/bottom/left/right/center; `LABEL_STYLE_ATTRS` + `BORDER_COLOR_ATTR` added; `labelAware()` applies them to gates and all label-bearing components (text: style keys only)
  - `src/tests/simulators/circuit/CircuitDesigner.test.tsx` - 6 new tests (corners not dashed rect, no center symbols, label location options, left label outside, label-style modal, border-color modal); text label attrs assertion updated for the new defaults
  - `src/tests/core/circuit/descriptors.test.ts` - AND/NOT `defaultAttrs` expectations updated with the 7 new keys

### [Added] - 2026-09-17 18:30
- **Component**: Circuit Designer — Multi-Selection (Rubber-Band Select) & Centered Pin Layout
- **Description**: Implemented Logisim-style multi-selection in the Circuit Designer. Left-dragging on empty canvas now draws a rubber-band box that captures **both components and wires**; releasing selects them (dragging a member of a multi-selection moves the whole group; Delete/Backspace removes the entire selection; Ctrl+R rotates it; the attributes panel shows a "N components selected" hint instead of a single table; the Edit menu Rename stays enabled only for a single selection). Panning moved from left-drag to right/middle-mouse drag or Space+left-drag (with right-click context menu suppressed on the canvas); an empty-canvas click or Escape deselects. The store gained `selection` + `selectedWires` arrays and batch actions `setSelection`, `moveComponents`, `removeComponents` (also prunes attached wires and both selection arrays), `removeWires`, and `rotateComponents`, each recording a single undo entry and reset by `createCircuit`/`switchTab`/`removeTab`/`clearCanvas`/`loadProject`/`undo`/`redo`. Separately, `centerRow` (layout.ts) was rewritten to spread any pin count symmetrically about the vertical centre (y=50, `gap = min(36, 60/(n-1))`, classic 2-pin 32/68 preserved) and both `ComponentGlyph.rowYs` and the duplicated logic in `SubcircuitGlyph` now delegate to it — previously n≥3 gates/subcircuits had off-centre, hardcoded pin rows.
- **Reasoning**: Multi-select (box selection of components + wires, group drag/delete/rotate) is a core Logisim interaction students expect for arranging circuits, and the existing left-drag pan conflict had to be resolved; per the user's decision, pan moved to right/middle/Space. The pin-layout fix removes a visible inconsistency where a gate's input pins were not vertically centred/equally spaced (three duplicated formulas disagreed with each other).
- **Impact**: Backward compatible for existing projects; single-click select/drag, wire placement, poke, attributes editing, undo/redo and the keyboard shortcuts Ctrl+Z/Y and Ctrl+R all behave as before. Selection state lives alongside the existing single `selected`/`selectedWire` primaries (kept as the first element of the batch). Verified: `npm run build` exit 0, lint clean (0 errors in changed files), all 38 CircuitDesigner tests + new store/`centerRow` tests pass, and the full suite's only failures remain the pre-existing `src/tests/simulators/numbersystems/` files (50) — unchanged.
- **Files Modified**:
  - `src/simulators/circuit/layout.ts` - `centerRow` rewritten: centred, symmetric, equal-spaced for any count
  - `src/simulators/circuit/ComponentGlyph.tsx` - `rowYs` delegates to `centerRow`
  - `src/simulators/circuit/CircuitDesigner.tsx` - box-select gestures, multi-drag/multi-delete/multi-rotate, wire + member click handling, right/middle/Space pan, empty-click/Escape deselect, box-select rect render, multi-selection attribute hint; `SubcircuitGlyph` uses `centerRow`
  - `src/stores/circuitStore.ts` - `selection`/`selectedWires` state + `setSelection`/`moveComponents`/`removeComponents`/`removeWires`/`rotateComponents`; `select`/`addComponent`/`removeComponent`/`removeWire` and all reset actions keep the arrays in sync
  - `src/tests/simulators/circuit/layout.test.ts` - new `centerRow` tests (symmetry, classic 2-pin, in-box, scaling)
  - `src/tests/simulators/circuit/CircuitDesigner.test.tsx` - box-select, group-drag, Delete, Escape, empty-click deselect, right-drag and Space+left-drag pan, multi-hint tests; `beforeEach` resets the new selection fields
  - `src/tests/stores/circuitStore.phase4.test.ts` - multi-selection store action tests

### [Updated] - 2026-09-17 12:20
- **Component**: Variable Name Editor Position — Single Row with Toolbar Buttons
- **Description**: Moved the variable-name editor from its own dedicated row below the toolbar into the main toolbar row itself, so it sits alongside the `Variables:`/`Value:` controls and the `Options`/`Clear` buttons on a single row. Also added `E` to `PRESET_VARIABLE_NAMES` so the default fifth variable renders as a normal dropdown like variables 1–4 (previously `E` fell back to the "Other" custom input, which broke the visual pattern).
- **Reasoning**: Requested by the user — the names editor should occupy the same single row as the buttons; the fifth variable should follow the same dropdown pattern as the other variables.
- **Impact**: Layout-only for position; functional change is `E` becoming a preset, which makes the default 5-variable name display as a dropdown (behavior otherwise unchanged — naming, validation, count-scaling and value preservation untouched). The row flex-wraps on narrow screens for responsive behavior. All 70 K-map tests pass; build and lint clean.
- **Files Modified**:
  - `src/simulators/kmap/components/KMapToolbar.tsx` - editor moved into the top toolbar row (next to PrimaryToolbar/ButtonGroup); separate bottom row removed
  - `src/simulators/kmap/components/variableNamePresets.ts` - added `E` to the preset list (after `D`)
  - `src/tests/simulators/kmap/VariableNameEditor.test.tsx` - 5-variable test now uses default `['A','B','C','D','E']` and asserts the 5th dropdown exists

### [Updated] - 2026-09-17 12:05
- **Component**: Variable Name Editor — Always Visible, Single-Row Layout
- **Description**: Removed the `+ Names`/`− Names` toggle button; the variable-name editor is now permanently visible on its own row below the primary toolbar. It renders a single-row `Name:` prefix followed by one compact select per variable position (matching the current 2/3/4/5 count), so all variable dropdowns sit on one line. Each dropdown lists the full preset list (`A B C D W X Y Z P Q R S M N L K`) plus **Other** for a custom single-letter name, with preset names already assigned elsewhere disabled. Validation errors appear inline next to the affected position.
- **Reasoning**: Requested by the user — names should always be visible without requiring a toggle, and all variable name selectors should be in a single row.
- **Impact**: No functional change to naming/validation behavior: dropdown count still tracks the variable count, custom names still limited to single letters, duplicates/empty still rejected inline. `CompactSelect` gained an optional `aria-label` prop for accessible per-position labels; the previous stacked "Variable N" labels were replaced by the single `Name:` prefix (labels preserved via aria-label). The commented-out 5-variable layout feature remains preserved as a comment block and is unaffected.
- **Files Modified**:
  - `src/simulators/kmap/components/KMapToolbar.tsx` - removed Names toggle button and `showNames` state; editor now always rendered
  - `src/simulators/kmap/components/VariableNameEditor.tsx` - reworked to single-row layout (`Name:` prefix + inline selects/custom input/inline errors)
  - `src/simulators/kmap/components/CompactSelect.tsx` - added optional `aria-label` prop
  - `src/tests/simulators/kmap/VariableNameEditor.test.tsx` - updated render/label assertions to the new single-row layout

### [Updated] - 2026-09-17 11:45
- **Component**: 5-Variable K-Map Layout Feature (Commented Out)
- **Description**: The 5-variable layout picker (plane variable dropdown + "Rows B C · Cols D E / Rows D E · Cols B C" swap control) is now commented out in `KMapToolbar.tsx` and its props removed from the `KMapSimulator.tsx` usage. The code is intentionally **not deleted** so it can be re-enabled later; only the user-facing control is hidden. Five-variable maps now always use the default plane layout (plane = first variable, rows = middle two, cols = last two).
- **Reasoning**: Requested by the user to simplify the 5-variable experience by hiding the layout feature while preserving the code for future re-enablement.
- **Impact**: None on K-map mathematics — plane/row/col axis state handling in `KMapSimulator.tsx` (plane var sync, axis-sync effect) is left intact and defaults to the standard layout, so renames and count changes behave exactly as before. All existing K-map tests pass.
- **Files Modified**:
  - `src/simulators/kmap/components/KMapToolbar.tsx` - import, props interface, destructuring, and render block for `FiveVarAxisControls` commented out
  - `src/simulators/kmap/KMapSimulator.tsx` - the `showAxisLayout`/`planeVar`/`swapAxes` props passed to `KMapToolbar` commented out

### [Added] - 2026-09-17 11:30
- **Component**: Dynamic Variable Name Selection in the K-Map Simulator
- **Description**: Added a variable-name editor so users can choose the names used everywhere in the K-map UI (grid axis labels, truth-table headers, SOP/POS, minterm/maxterm, simplified expression, groups, walkthrough, PDF export) instead of being locked to A/B/C/D. A per-variable dropdown offers the common presets `A B C D W X Y Z P Q R S M N L K` plus an **Other** option that reveals a compact custom-name input. Names are validated as a batch (trimmed, non-empty, single letter per the expression engine's restriction, case-insensitively unique, 2–5 count); used presets are disabled per dropdown and invalid custom input shows inline messages and never reaches the model. Variable-count changes now preserve existing selections where possible (growing appends positional defaults `D`/`E` etc. skipping used letters; shrinking removes trailing variables cleanly), and entered cell values are preserved across renames/count changes via minterm bit-shift repainting.
- **Reasoning**: Students and instructors commonly want the K-map to mirror the variables of their particular problem (e.g. `X, Y, P` or `F, A, B, C`) rather than always reading `A, B, C, D`. The core engine already maps positional variable index → name at expression-generation time, so names work as a pure presentation mapping with zero changes to the K-map mathematics.
- **Impact**: Backward compatible. Default behaviour is byte-identical: no customization keeps `A,B,C`, count changes reproduce the legacy presets exactly, and simplification/grouping/gray-code/minterm math is untouched. The application layer gained `validateVariableNames`, `adjustVariablesToCount`, and `renameKMapVariables`; the store's `setVariables` now repaints cell values instead of wiping them (a UX improvement — renaming no longer clears the entered map, and 4→3 keeps the D=0 half by shifting minterms right). New 43-test coverage: application validators + count-scaling + bit-shift preservation + custom-name expression output, editor UI interactions (preset commit, Other/custom commit, duplicate/empty/single-letter errors, used-preset disabling, external reconciliation), and store integration. All K-map/application/store suites pass (566 tests); `tsc -b` clean, `vite build` OK, lint 0 errors in changed files. The full suite's only failures remain the pre-existing `src/tests/simulators/numbersystems/` Octal/visualizer files documented in the 2026-09-17 10:50 audit entry — unrelated and unchanged.
- **Files Modified**:
  - `src/application/kmap/variable-names.ts` - NEW: `validateVariableNames` (batch, trimmed/single-letter/case-insensitive-unique/2-5), `adjustVariablesToCount` (name-preserving grow/shrink with positional defaults), `renameKMapVariables` (rebuild model + repaint minterm→value with bit shift)
  - `src/application/kmap/index.ts` - Barrel exports the new module
  - `src/stores/kmapStore.ts` - `setVariables` now preserves entered cell values via `renameKMapVariables`
  - `src/simulators/kmap/components/VariableNameEditor.tsx` - NEW per-variable editor (preset dropdown + Other custom input + inline validation + used-name disabling)
  - `src/simulators/kmap/components/variableNamePresets.ts` - NEW preset letter list (kept separate for fast-refresh)
  - `src/simulators/kmap/components/KMapToolbar.tsx` - `+ Names` toggle panel rendering the editor
  - `src/simulators/kmap/KMapSimulator.tsx` - count change uses `adjustVariablesToCount`; new `handleVariablesChange` (validate + apply + 5-var plane sync)
  - `src/simulators/kmap/components/CompactSelect.tsx` - optional `id` and per-option `disabled` support
  - `src/tests/application/kmap/variable-names.test.ts` - NEW validator/count/rename/expression-flow suite
  - `src/tests/stores/kmapStore.test.ts` - NEW store integration suite
  - `src/tests/simulators/kmap/VariableNameEditor.test.tsx` - NEW editor UI suite
- **Success Criteria Met**:
  - 2-var A,B and 3-var A,B,C defaults unchanged ✓
  - 2-var X,Y and 3-var X,Y,Z produce X/Y/Z-labelled maps and expressions (`X'Y + XY'`, `X'Y'Z' + XYZ`) ✓
  - Custom "Other" single-letter names validated and applied everywhere ✓
  - Duplicate (incl. case-insensitive) and empty custom-name validation with inline messages; invalid sets never applied ✓
  - Count changes 4→3 and 3→4 preserve names where possible with defaults added / trailing removed ✓
  - Simplification produces identical mathematical results (no core math changes) ✓
  - All existing tests pass; full K-map/application/store suites + 43 new tests green ✓

### [Fixed] - 2026-09-17 10:50
- **Component**: K-Map 5-Variable Math Audit — Adjacency, Group Validation, Plane-Aware Model Creation & Educational Module
- **Description**: Systematically corrected every 5-variable K-map module that still treated the stacked-plane model (4×4×2) as a flat 2-D grid:
  - **`src/core/kmap/adjacency.ts`** (rewritten): adjacency is now defined by the Boolean cube — two cells are adjacent iff their minterms differ in exactly one variable (Hamming distance 1) — computed over the axis indices (row, column-within-plane, plane) instead of the flattened grid. This fixes: cross-plane adjacency (m0 and m16 are now adjacent, `'plane'` direction), the horizontal wrap ring per plane (a 4-column ring, not the flattened 8-column ring), and the false "seam" adjacency between the two planes on the flattened grid. `AdjacencyDirection` gained `'plane'`; `neighborsOf(kmap, row, col)` keeps its grid-coordinate signature but resolves the center's minterm then returns Hamming neighbours (so each 5-var cell has exactly 5 neighbours: 4 in-plane incl. wrap + 1 mirrored across the fold). 2–4 variable behaviour is byte-identical (one-bit perm of the same definition).
  - **`src/core/kmap/grouping.ts`**: `validateGroup` now validates a group as a subcube of the three axes — a cyclic-contiguous power-of-two block on rows × columns-within-plane × planes whose cell set is exactly the Cartesian product R×C×P. It accepts intra-plane wrap pairs (`[0,2]`, `[0,8]`) and cross-plane mirror groups (`[0,16]`, the mirrored octet) that the old flat-grid validator rejected, and still rejects L-shapes, diagonals, non-powers-of-two and out-of-range cells (degenerates identically for `planes === 1`). Internal `occupiedAxesIncludingPlanes` added; exported `occupiedAxes` semantics preserved.
  - **`src/application/kmap/use-cases.ts`**: `createKMapWithVariables` now builds a true plane model for exactly 5 valid variables (`buildAssignment([variables[0]], next two, last two)`) instead of a transient flat 4×8 grid, so simplification is minimal from the moment the model exists (the simulator previously corrected this in an effect). 2–4 variable and legacy flat `createKMap` defaults are unchanged.
  - **`src/core/kmap/five-variable.ts`**: `analyzeFiveVariable`, `planeVariableOf`, and `planeFourVarTerm` are now layout-aware — the plane variable is read from the model's plane axis (or, for a legacy flat model, the last column variable, matching `FiveVarGrid`'s virtual-plane derivation), `e`/`abcd` are derived from that variable's canonical bit position rather than a hard-coded "E = MSB" split; `splitFive` kept as the documented canonical E-MSB helper. Educational text now uses the actual plane variable name.
- **Reasoning**: The Phase-4/5 plane-first model made `simplify`, grid rendering and `cellToMinterm`/`mintermToCell` correct, but three remaining modules still read the flattened 4×8 grid: adjacency reported a non-adjacent cross-map pair (m0-m16) as not adjacent, applied the wrong wrap ring past the plane seam, group validation rejected legitimate 5-variable groups (wrapped in-plane pairs and mirrored across-the-fold groups), the educational 4+1 module hardcoded E and A–D names, and application-level model creation transiently produced non-minimal flat models. These were inconsistent with both the mathematical definition of K-map adjacency and the user-requested plane-first invariant (plane variable = MSB).
- **Impact**: 5-variable K-maps are now mathematically consistent end-to-end: adjacency/hover highlight, group validation (exercise/solution checking), simplification grouping, and the educational 4+1 breakdown all operate on the same cube-adjacency semantics. No behavioural change for 2–4 variable maps (same function shapes, flat path proven identical). New test suite `src/tests/core/kmap/five-variable-math.test.ts` (29 tests) pins the plane-A ground-truth grid, per-cell 5-neighbour adjacency invariants, cross-plane/wrap groups, minimal SOP covers of every size (1…32 cells, in-plane, wrapped, merged octet, don't-care enlargement, multi-term cover), and validateGroup accept/reject. All K-map, application and education suites pass. Full suite 1802 passed — the only 51 failures are the pre-existing flaky `src/tests/simulators/numbersystems/` Octal files (verified identical on clean HEAD with the change stashed). `tsc -b` clean, `vite build` OK, lint 0 errors in changed files.
- **Files Modified**:
  - `src/core/kmap/adjacency.ts` - Rewritten: Hamming-distance-1 adjacency over axis indices; `'plane'` direction; per-plane wrap ring; `neighborsOf` grid signature retained
  - `src/core/kmap/grouping.ts` - `validateGroup`/`isSubcube` 3-axis subcube validation; internal `occupiedAxesIncludingPlanes`; exported `occupiedAxes` preserved
  - `src/application/kmap/use-cases.ts` - `createKMapWithVariables` plane-aware for exactly 5 variables (plane = first variable, rows = next two, cols = last two)
  - `src/core/kmap/five-variable.ts` - Layout-aware `analyzeFiveVariable`/`planeVariableOf`/`planeFourVarTerm`; `splitFive` documented as canonical
  - `src/tests/core/kmap/five-variable-math.test.ts` - NEW 29-test 5-variable math suite (grid, adjacency invariants, grouping, minimal covers)
  - `src/tests/core/kmap/five-variable.test.ts` - Updated to the plane-A model + legacy-flat coverage test
- **Success Criteria Met**:
  - Cross-plane pair m0+m16 reported adjacent (`'plane'`) and simplifies to `B'C'D'E'` ✓
  - Horizontal wrap inside a plane on the 4-column ring (m0+m2 → `A'B'C'E'`) ✓
  - `validateGroup` accepts wrapped in-plane pairs and mirrored across-the-fold groups ✓
  - 5-var plane model built by default from `createKMapWithVariables` ✓
  - Educational module variable/layout independent (plane A–E and legacy flat all correct) ✓
  - Flat (planes = 1) adjacency/validation byte-identical; all kmap/application/education tests green ✓

### [Added] - 2026-09-17 10:20
- **Component**: K-Map Phase 5 — Standalone Plane-First Module (`src/core/kmap/plane.ts`)
- **Description**: New self-contained core module implementing the plane-first rule for 5-variable K-map layout: **whichever variable is chosen as the plane variable becomes the MSB**. `buildPlaneLayout(variables, planeVariable, options?)` produces a `KMapPlaneLayout` whose `order` is `[planeVariable, ...rowVars, ...colVars]` (index 0 = MSB, bit `n-1`), so plane 0 always holds plane-first minterms `0..15` and plane 1 always holds `16..31` regardless of which of the five variables separates the planes. Rows/columns are the remaining variables (first two = rows by default, `rowCount: 1..2` override) with standard Gray-code labels. Pure mapping helpers: `planeCellMinterm(layout, row, col, plane)` (plane variable is the MSB, plane-1 cells offset by `1 << (n-1)`), `planePosition(layout, minterm)` (inverse), `planeGrid`/`planeCells` (per-plane inspection grids), and `canonicalToPlaneSpace`/`planeSpaceToCanonical` (per-variable bit-slot translation between the user's canonical ordering and the plane-first ordering, e.g. canonical `00101` → plane-first `18` for the E plane). The module is deliberately independent of `model.ts` so the plane shape can be verified in isolation; it is exported from the kmap barrel but NOT wired into the simulator yet.
- **Reasoning**: The existing 5-variable grid treats the plane variable as the LSB internally (canonical `A..E` ordering) and compensates visually at render time. The user requested the plane be established first with the invariant that the plane variable is the MSB — this standalone module makes that invariant the single source of truth (plane 0 = 0..15, plane 1 = 16..31) and provides bijective cell↔minterm mapping plus canonical↔plane-first translation that the simulator can later be wired to.
- **Impact**: Additive, core-only, backward compatible. No existing module changed (only a new export in the kmap barrel); nothing is wired into `KMapSimulator`/`FiveVarGrid`, so existing layout rendering is untouched. 16 new tests in `src/tests/core/kmap/plane.test.ts` cover layout construction (default E plane, non-E plane variable C, 4-var support, validation errors), standard 4-var grid rendering in plane 0, plane-1 +16 offsets for every plane variable, py cells always within `0..15`/`16..31`, 32 unique minterms, position round-trips for all 32 minterms × all plane variables, and canonical↔plane-first translation round-trips. `npm test -- --run src/tests/core/kmap src/tests/application/kmap` → 425/425 pass (32 files); clean `tsc -b` and successful `vite build`.
- **Files Modified**:
  - `src/core/kmap/plane.ts` - NEW standalone plane-first module: `KMapPlaneLayout`, `buildPlaneLayout`, `planeCellMinterm`, `planePosition`, `planeGrid`, `planeCells`, `canonicalToPlaneSpace`, `planeSpaceToCanonical`
  - `src/core/kmap/index.ts` - Added `export * from "./plane"`
  - `src/tests/core/kmap/plane.test.ts` - NEW 16 tests (layout, grid, offsets, round-trips, translation, validation)
- **Success Criteria Met**:
  - Plane variable is always the MSB in the plane-first order (`order[0]`) ✓
  - Plane 0 = plane-first minterms `0..15`, plane 1 = `16..31` for every plane variable ✓
  - `planeCellMinterm`/`planePosition` bijective across all 32 minterms × all plane variables ✓
  - Canonical ↔ plane-first translation round-trips for every plane variable ✓
  - Standard 4-var grid shape preserved (plane 0 = `[[0,1,3,2],[4,5,7,6],[12,13,15,14],[8,9,11,10]]`) ✓
  - Not wired into simulator; existing kmap tests/build unchanged and green ✓

### [Added] - 2026-09-17 00:50
- **Component**: K-Map Phase 4 — Layout-Independent Simplification + Comprehensive Phase 1/2/3 Test Coverage
- **Description**: Root-caused and fixed a Phase-2 regression in `simplify` (`src/core/kmap/simplify.ts`): `enumerateRectangles` enumerated rectangle blocks over the *flattened* grid column axis (`layout.cols`), which for stacked-plane layouts (5-var plane assignment) could not represent group cells that cross the plane seam — so `simplify` produced layout-dependent results (e.g. `A'DE' + ADE'` for the plane-A layout vs flat `DE'`, and a wrongly-merged `B` group) while wrap detection also fired incorrectly across the plane axis. The enumerator was rewritten to enumerate per-axis: row blocks (all rows), column blocks (per-drawn-column-size `colCount = planes <= 1 ? layout.cols : layout.cols / planes`), and plane blocks (all planes), building each candidate's minterms via the existing `cellToMinterm(kmap, row, col, plane)` and merging identical column blocks across a power-of-two plane block — the mirrored fold that eliminates the plane variable. The flat path (`planes === 1`) degenerates to exactly the legacy enumeration, so all existing simplify/problem-generator/regression tests are byte-identical. New tests: `src/tests/core/kmap/assignment.test.ts` (17 tests) proving (a) round-trip `cellToMinterm`/`mintermToCell` bijection for every legal `buildAssignment` topology (plane 0/1 × rows 1–2 × cols 1–2 × every variable permutation for n = 3/4/5) and for the defaults n = 2–5, and (b) canonical-minterm invariance — identical Σm input yields identical normalized SOP/POS across every layout for default-representable primes (`A'B'C'`, `A`, `B`, plus don't-care and mixed-term fixtures), with a pinned boundary bucket: true plane layouts must agree on `E'`/`D` (the flat default's folded column axis cannot draw those as single rectangles, and its legacy two-term cover is asserted unchanged). It also removes the internal helper `rectangleCells`. Component coverage for the new invariants was added to `FiveVarGrid.test.tsx` (+12 tests, 26 total): plane labels (`{planeVar} = 0/1`) and row/col variable headers for every plane variable A–E, every cell's drawn `x`/`y` position derived from its `mintermToCell` slot (one `x` value per 4-col plane, correct plane-gap offset), cells stacked across the gap differing in exactly the plane-variable bit, and parametrized wrap-group overlay tests per plane variable (plane-0 wrap = two merged full-height bars; cross-plane wrap = four bars in both planes).
- **Reasoning**: The Phase 4 spec demands two proofs — roundtrip for every assignment and canonical-minterm invariance. Both are false on the current codebase because of the flatten-axis enumeration, so fixing `simplify` is a prerequisite, not a scope creep: without it the invariance test fails on the current code with the layout-dependent covers above. The fix stays in the core layer, uses only existing public mapping functions, and provably preserves the flat path's output.
- **Impact**: `simplify` results are now identical for the same minterm set under every legal axis assignment (default flat layouts and all plane/row/col permutations) for default-representable functions; the folded-axis limitation for column-axis-constant primes (E'/D) is now explicit, documented, and pinned by tests. Backward compatible: flat `planes === 1` enumeration is structurally identical to the legacy path, and all 232 remaining core tests plus the 27-file kmap suites pass unchanged. kmap suites: 28 files / 377 tests all passing (+29 new: 17 core + 12 component). `tsc -b` clean; lint 0 errors (the 16 warnings are pre-existing, none in changed files); `vite build` OK. Full suite: 1626 passed / 51 failed — every failure is in the pre-existing flaky `src/tests/simulators/numbersystems/` suite (same 8 files, ±3 timing fluctuation vs the 48 base) — none related to this change. The temporary `__scratch.test.ts` experiment file was moved to `trash/src/tests/core/kmap/` (documented in `trash/trash.md`).
- **Files Modified**:
  - `src/core/kmap/simplify.ts` - Rewrote `enumerateRectangles` to per-axis (row × per-plane-column × plane) enumeration via `cellToMinterm`; `planes === 1` degrades to the legacy flat enumeration; removed the `rectangleCells` helper; imported `cellToMinterm` from `./model`
  - `src/tests/core/kmap/assignment.test.ts` - NEW: roundtrip for defaults n = 2–5 and every legal assignment topology for n = 3/4/5; canonical-minterm invariance across all layouts; don't-care and mixed-term fixtures; E'/D boundary bucket (plane layouts agree on the minimal cover; legacy flat two-term cover pinned)
  - `src/tests/simulators/kmap/FiveVarGrid.test.tsx` - +12 tests: plane labels + variable headers for plane A–E, per-plane cell positions from `mintermToCell`, plane-bit-only difference across the gap, parametrized wrap overlays per plane variable
  - `trash/trash.md` - Documented the moved scratch file
- **Success Criteria Met**:
  - Round-trip `cellToMinterm`/`mintermToCell` verified for every assignment topology ✓
  - Canonical-minterm invariance: same Σm → same normalized simplification under every layout (stricter than "no worse than the default") ✓
  - `FiveVarGrid` plane labels + cell numbers correct for plane selection A/B/C/D/E ✓
  - Wrap-around groups render per assignment (parametrized over every plane variable) ✓
  - Regression: default assignment behavior byte-identical (all pre-existing kmap tests pass unchanged) ✓

### [Added] - 2026-09-17 00:35
- **Component**: K-Map Phase 3 — Presentation (FiveVarGrid axis-derived rendering, toolbar Layout control, wrap-overlay tests)
- **Description**: `FiveVarGrid` no longer special-cases flat vs plane layouts: for flat models (`planes === 1`) a lightweight virtual plane model is derived via the new `derivePlaneModel`, which takes the last column variable as the plane variable and rebuilds rows/columns from the remaining variables (`buildAssignment([planeVar], rows, cols)` + `createKMap`), so every 5-var model renders through the same axis-derived `PlaneLayout` driver. Plane labels now render `{planeVar} = 0` / `{planeVar} = 1` instead of hardcoded `E = 0` / `E = 1`, and the plane bit comes from `mintermToCell(renderModel, m).plane` — the `m & 1` LSB hack and the legacy `planeCellMinterm`/`fromGrayCode` paths were removed entirely. Group overlays filter by `posOf(m).plane` (the actual plane variable's bit, e.g. C at bit 2, not bit 0) while keeping the `contiguousRuns` wrap rendering that works on axis indices. The Layout control moved from the grid card into the existing toolbar: `FiveVarAxisControls` was rewritten to reuse the project's `CompactSelect` (plane-variable dropdown) and `SegmentedControl` (`sm`, labels showing the live rows/columns split as `Rows A B · Cols C D` vs swapped) — no new panel, reusing existing UI primitives. `KMapToolbar` accepts new optional props (`showAxisLayout`, `variables`, `planeVar`, `onPlaneVarChange`, `swapAxes`, `onSwapAxesChange`) and renders the control in the primary flex row only for 5 variables; the inline selector block was removed from `KMapSimulator`, which now passes `activePlaneVar`/`activeSwap` down to the toolbar. New tests lock down wrap-around group overlays under a non-default plane assignment (plane C, rows AB, cols DE): a left-right wrap group in the C=0 plane renders as two merged bars at that plane's offset, a group split across both planes renders its bars in each plane filtered by the real plane bit (not the LSB), and a non-wrapping C=1 group renders a single merged rectangle.
- **Reasoning**: Completes the Phase 3 presentation milestone so the simulator's rendering is fully driven by the model's axis assignment rather than hardcoded E/bit0 assumptions, making any plane/row/col assignment render correctly with no duplicated legacy math. Reusing `CompactSelect`/`SegmentedControl` puts the compact "Layout" control inside the existing toolbar per the Phase 3 spec, and the wrap-group tests validate that the `contiguousRuns` strategy (axis-index based, assignment-agnostic) correctly renders seam-decomposed rectangles under a custom plane variable.
- **Impact**: Additive, backward compatible, presentation-only. Default 5-var behavior is unchanged: the flat model derives to the virtual E-plane layout (E labels, cells, click minterms, overlaid geometry all identical — existing flat-model `FiveVarGrid` tests pass unmodified through `derivePlaneModel`). TypeScript build clean; lint 0 errors (no new warnings; the 16 warnings are pre-existing). All 27 kmap test files pass (348 tests, incl. 3 new wrap-overlay tests). Full suite: 1600 passed / 48 failed — every failure is in the pre-existing flaky `src/tests/simulators/numbersystems/` suite (48 at base) — none related to this change.
- **Files Modified**:
  - `src/simulators/kmap/components/FiveVarGrid.tsx` - Added `derivePlaneModel` (virtual plane model for flat layouts); `usePlaneLayout` now renders any 5-var layout wholly from model axes; plane labels show `{planeVar} = {plane}`; posOf/cellMinterm via `mintermToCell`/`cellToMinterm`; removed `planeCellMinterm` and the `fromGrayCode`/`(m & 1)` legacy math
  - `src/simulators/kmap/components/FiveVarAxisControls.tsx` - Rewritten to reuse `CompactSelect` + `SegmentedControl` (segments show the live rows/cols split); same prop API
  - `src/simulators/kmap/components/KMapToolbar.tsx` - New optional `showAxisLayout`/`variables`/`planeVar`/`onPlaneVarChange`/`swapAxes`/`onSwapAxesChange` props; renders `FiveVarAxisControls` inside the primary toolbar row for 5 variables
  - `src/simulators/kmap/KMapSimulator.tsx` - Removed the inline `FiveVarAxisControls` block from the grid card; threads `activePlaneVar`/`activeSwap` through `<KMapToolbar>`
  - `src/tests/simulators/kmap/FiveVarGrid.test.tsx` - 3 new tests for wrap-around group overlays under a custom plane assignment (merged bars per seam, per-plane filtering by real plane bit, single merged rect for non-wrapping group)
- **Success Criteria Met**:
  - Plane labels render `<planeVar> = 0` / `<planeVar> = 1` (no hardcoded E) ✓
  - No `m & 1` in grid rendering — plane bit via `mintermToCell` ✓
  - Compact "Layout" control (plane dropdown + swap SegmentedControl) lives in the existing toolbar ✓
  - `contiguousRuns` wrap rendering preserved and locked down with tests under non-default assignments ✓
  - Default flat 5-var model renders identically (all existing `FiveVarGrid` tests pass via `derivePlaneModel`) ✓
  - kmap tests 348/348 pass; tsc clean; lint 0 errors; build OK ✓

### [Added] - 2026-09-16 23:52
- **Component**: K-Map Phase 2 — Application `applyAssignment` + Presentation-Driven 5-Variable Plane Axis Selectors
- **Description**: Added `applyAssignment(model, assignment)` to the application layer: it validates the axis assignment against the model's full variable set (via `createKMap`'s coverage check), rebuilds the model with the new axes, and carries over every non-null cell value by minterm. Exported through `src/application/kmap` like the other use-cases. On the presentation side, the 5-variable simulator now has an axis selector row. `FiveVarAxisControls` (new component) offers a "Plane:" dropdown to choose which of the five variables separates the two planes (default E) and a "Swap Rows ⇄ Cols" toggle (default rows = first two remaining variables in gray order, columns = last two), plus a live readout of the resulting rows/cols split. `KMapSimulator` owns the `planeVar`/`swapAxes` state, derives the desired `buildAssignment`, and a `useEffect` compares the current model axes with the desired axes and calls `applyAssignment` only when they differ — so painting cells, clearing, or loading examples never rebuilds unnecessarily and all grid values survive layout changes. `FiveVarGrid` was refactored around a `PlaneLayout` driver: plane models (layout.planes > 1) derive plane variable, row/column variables, cell-minterm mapping (`cellToMinterm`) and position lookup (`mintermToCell`) directly from the model axes, so a custom plane/row/column assignment renders consistently (correct plane labels, row/col variable headers, cross-plane lines, and per-plane group overlay filtering using the actual plane bit rather than assuming E = bit0). The legacy flat 5-var path (planes === 1, E folded into the column axis) keeps its original hard-coded mapping byte-for-byte, so existing behavior and tests are unaffected.
- **Reasoning**: Phase 1 delivered the axis-assignment core model and opt-in plane layouts; Phase 2 makes that capability usable. Students can now view the same 5-variable function grouped by any variable (e.g. C-planes instead of E-planes) and reorient rows/columns, which strengthens the connection between variable choice and 5-variable grouping — and the application-layer orchestration keeps the presentation slice thin, staying within the 4-layer architecture.
- **Impact**: Additive and backward compatible. Default behavior is unchanged except that selecting 5 variables now re-renders the 5-var grid from the model's default E-plane assignment (same E=0/E=1 planes as before). Simplification, truth table, walkthrough, grouping, adjacency highlighting and PDF export all consume the model's minterm mapping, so they remain correct for every layout. TypeScript build clean; lint 0 errors (no new warnings); all 27 kmap test files pass (345 tests, incl. 10 new: 6 `applyAssignment` + 4 custom-plane `FiveVarGrid`). Full suite: 1596 passed / 49 failed, where every failure is in the pre-existing flaky `src/tests/simulators/numbersystems/` suite (48 at base; ±1 timing fluctuation) — none related to this change.
- **Files Modified**:
  - `src/application/kmap/use-cases.ts` - Added `applyAssignment(model, assignment)` (validation + value-preserving rebuild), exported via the existing barrel
  - `src/simulators/kmap/components/FiveVarAxisControls.tsx` - New compact selector component (Plane dropdown, Swap Rows ⇄ Cols toggle, split readout)
  - `src/simulators/kmap/components/FiveVarGrid.tsx` - Refactored around a `PlaneLayout` driver; plane models render from model axes (plane label, row/col headers, group overlay plane filtering), legacy flat path preserved
  - `src/simulators/kmap/KMapSimulator.tsx` - Added `planeVar`/`swapAxes` state, reset on variable-count change and example load, axes-sync effect via `applyAssignment`, renders the selector row for 5 variables
  - `src/tests/application/kmap/use-cases.test.ts` - 6 new tests (plane rebuild, value preservation, plane-variable change, swap, coverage validation errors, 4-var custom assignment)
  - `src/tests/simulators/kmap/FiveVarGrid.test.tsx` - 4 new custom-plane layout tests (plane labels, value placement, swapped axes, click minterm)

### [Added] - 2026-09-16 23:29
- **Component**: K-Map Phase 1 — Explicit Axis Assignment Core Model
- **Description**: Generalized `KMapLayout` from the implicit `rowVariables`/`colVariables` split into an explicit, validated axis assignment. New `axes: { kind: 'plane' | 'row' | 'col'; variables: string[] }[]` field on `KMapLayout` is the single source of truth for how a minterm's bits map onto the grid. Added the pure mapping functions `cellToMinterm(model, row, col, planeIndex)` and `mintermToCell(model, minterm)` — both derived from the axis assignment: for each axis `toGray(axisIndex)` is written at (or read from) that axis's variables' canonical bit positions. `mintermToCell` returns `{ row, col, plane? }` (the `plane` field is present only when the layout actually has a plane axis, so single-plane layouts keep the exact legacy `{ row, col }` shape). Added `buildAssignment(planeVars, rowVars, colVars)` with validation: every variable in exactly one axis, max 2 variables per axis, exactly one row and one col axis, at most one plane axis, 2–5 total variables, and (in `createKMap`) the assignment must cover the map's variables exactly. `createKMap(variables, assignment?)` now accepts an optional assignment; without one it defaults to the exact current behavior for 2/3/4/5 variables (5-var stays a flat 4×8 grid with E folded into the column axis at bit0). A plane assignment (e.g. `buildAssignment(['E'], ['A','B'], ['C','D'])` for 5 vars, or `['D'], ['A','B'], ['C']` for 4) produces a true plane layout with `layout.planes` and plane-major flattened columns; `valueAt`/`withValue`/`cellToMinterm`/`mintermToCell` all round-trip correctly across planes.
- **Reasoning**: To prepare the K-map simulator for configurable axis layouts (planes for 4/5 variables, custom row/column variable splits) the model previously hard-coded the row/col split inside `mintermToCell`/`cellToMinterm` and encoded 5-variable planes only conceptually in the five-variable module. An explicit validated axis assignment makes the mapping math the single source of truth and supports opt-in plane modes without disturbing the default layout.
- **Impact**: Fully backward compatible. Default `createKMap` output (axes-derived `rowVariables`/`colVariables`, rows/cols, col/row labels, cells, minterms) is identical to before for every variable count; no consumer of `rowVariables`/`colVariables`/`mintermToCell` (`KMapGrid`, `FiveVarGrid`, `pdfExport`, grouping, adjacency, five-variable module) had to change. TypeScript build clean; lint 0 errors; all 27 kmap test files pass (335 tests, incl. 26 new model tests). The 48 failures in `src/tests/simulators/numbersystems/` are pre-existing (identical on the base commit) and unrelated to this change.
- **Files Modified**:
  - `src/core/kmap/model.ts` - Added `AxisKind`, `KMapAxis`, `KMapAxisAssignment`, `layout.axes`, `layout.planes`, `layout.planeVariables`, `buildAssignment`; rewrote `mintermToCell`/`cellToMinterm`/`createKMap`/`valueAt`/`withValue` around the axis assignment
  - `src/tests/core/kmap/model.test.ts` - Added 26 tests: default axes for 2/3/4/5 vars, `buildAssignment` validation, 5-var plane layout grid/labels/round-trips/values, custom 4-var row/col assignment, optional 4-var plane assignment
