 ...
 * Teaches the mathematical calculation using powers of 2
 */
...
export function Lesson4_UnderstandingPosition({ onComplete }: Lesson4Props) {
  ...
  const number = [1, 0, 1] // 101 in binary = 5 in decimal
  const positionNames = ['Fours', 'Twos', 'Ones']
  const placeValues = [4, 2, 1]
  const powersOf2 = ['2²', '2¹', '2⁰']
      ...
      <div className="text-center space-y-6">
        ...
        <div className="p-4 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
          ...
          <p className="text-base mt-2" style={{ color: 'var(--text-secondary)' }}>
            Let's calculate it step by step for 101 (binary)
          </p>
        </div>
        ...
        <div className="space-y-4">
          {!isComplete && (
            ...
            >
              {state.currentTier === 0 ? 'Show Positions' : state.currentTier === 1 ? 'Show Place Values' : 'Show Powers of 2'}
            </Button>
          )}
          ...
          {state.currentTier >= 1 && (
            <div className="animate-fade-in p-4 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
              <p className="text-base" style={{ color: 'var(--text-secondary)' }}>
                <strong style={{ color: 'var(--text-primary)' }}>Step 1:</strong> Each position has a name - Fours, Twos, Ones
              </p>
            </div>
          )}
          ...
          {state.currentTier >= 2 && (
            <div className="animate-fade-in p-4 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
              <p className="text-base" style={{ color: 'var(--text-secondary)' }}>
                <strong style={{ color: 'var(--text-primary)' }}>Step 2:</strong> Each position has a place value - 4, 2, 1
              </p>
            </div>
          )}
          ...
          {state.currentTier >= 3 && (
            <div className="animate-fade-in p-4 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
              <p className="text-base" style={{ color: 'var(--text-secondary)' }}>
                <strong style={{ color: 'var(--text-primary)' }}>Step 3:</strong> Each place value is a power of 2 - 2², 2¹, 2⁰
              </p>
            </div>
          )}
          ...
          {isComplete && (
            <div className="animate-fade-in-up p-6 rounded-xl" style={{ backgroundColor: 'var(--accent-bg)' }}>
              <p className="text-lg font-bold" style={{ color: 'var(--accent-primary)' }}>
                Digit → Position → Power of 2 → Place Value → Contribution
              </p>
            </div>
          )}
          ...
          {isComplete && (
            <div className="animate-fade-in p-6 rounded-xl" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
              <h3 className="text-xl font-bold mb-4" style={{ color: 'var(--text-primary)' }}>
                Complete Calculation: 101 (binary) = 5 (decimal)
              </h3>
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 rounded-lg" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                  <span style={{ color: 'var(--text-secondary)' }}>1 × 4 =</span>
                  <span className="font-bold" style={{ color: 'var(--accent-primary)' }}>4</span>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                  <span style={{ color: 'var(--text-secondary)' }}>0 × 2 =</span>
                  <span className="font-bold" style={{ color: 'var(--accent-primary)' }}>0</span>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                  <span style={{ color: 'var(--text-secondary)' }}>1 × 1 =</span>
                ...
                <div className="flex items-center justify-between p-3 rounded-lg border-t-2" style={{ backgroundColor: 'var(--accent-bg)', borderColor: 'var(--accent-primary)' }}>
                  ...
                  <span className="font-bold text-xl" style={{ color: 'var(--accent-primary)' }}>5</span>
                </div>
              </div>
            </div>
          )}
