 ...
 * Game-like challenges to test understanding of L1-L4 concepts for octal
 */
...
export function Lesson5_ChallengeMode({ onComplete }: Lesson5Props) {
  ...
  const challenges = [
    ...
    },
    {
      id: 4,
      type: 'builder',
      question: 'Build the octal number',
      target: 73, // 111 in octal = 73 in decimal
      positionNames: ['Sixty-fours', 'Eights', 'Ones']
    }
  ]
  ...
  const handleBuilderDigitChange = (position: number, value: number) => {
    ...
    if (currentChallengeData.type === 'builder') {
      const builtNumber = newDigits[0] * 64 + newDigits[1] * 8 + newDigits[2] * 1
    ...
    <div className="space-y-8">
      ...
      <div className="text-center space-y-6">
        ...
        <p className="text-base" style={{ color: 'var(--text-secondary)' }}>
          Test everything you learned in Lessons 1-4 about octal!
        </p>
            ...
            {currentChallengeData.type === 'builder' && currentChallengeData.positionNames && (
              <div className="space-y-4">
                ...
                <div className="p-4 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
                  ...
                  <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>Your octal: {state.builderDigits.join('')}</p>
                  <p className="text-2xl font-bold mt-1" style={{ color: 'var(--accent-primary)' }}>
                    Calculated: {state.builderDigits[0] * 64 + state.builderDigits[1] * 8 + state.builderDigits[2] * 1}
                  </p>
                </div>
            ...
            <div className="p-8 rounded-xl" style={{ backgroundColor: 'var(--accent-bg)' }}>
              ...
              {state.correctAnswers === challenges.length && (
                <p className="text-lg mt-2" style={{ color: 'var(--text-secondary)' }}>
                  Perfect score! You've mastered octal number systems!
                </p>
              )}
            </div>
