...
export function Lesson3_PositionalSystem({ onComplete }: Lesson3Props) {
  ...
  const number = [1, 1, 1] // 111 in octal
  const positionNames = ['Sixty-fours', 'Eights', 'Ones']
  const positionValues = [64, 8, 1]
    ...
    <div className="space-y-8">
        ...
        {state.showSumAnimation && (
          <div className="animate-fade-in-up space-y-4">
            <div className="flex items-center justify-center gap-2 text-2xl">
              ...
              <span style={{ color: 'var(--text-secondary)' }}>=</span>
              <span className="font-bold" style={{ color: 'var(--accent-primary)' }}>73</span>
            </div>
            ...
            <div className="p-4 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
              ...
              <div className="mt-2 space-y-1">
                <p style={{ color: 'var(--text-primary)' }}>• 1 in ones place = 1</p>
                <p style={{ color: 'var(--text-primary)' }}>• 1 in eights place = 8</p>
                <p style={{ color: 'var(--text-primary)' }}>• 1 in sixty-fours place = 64</p>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="mt-8 p-6 rounded-xl" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
        ...
        <p className="text-sm mt-2" style={{ color: 'var(--text-muted)' }}>
          💡 Next lesson: We'll learn HOW position value is calculated using powers of 8!
        </p>
      </div>
    </div>
  )
