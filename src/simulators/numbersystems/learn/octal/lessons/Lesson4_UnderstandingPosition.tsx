 ...
 * Teaches the mathematical calculation using powers of 8
 */
...
export function Lesson4_UnderstandingPosition({ onComplete }: Lesson4Props) {
  ...
  const number = [3, 4, 5] // 345 in octal = 229 in decimal
  const positionNames = ['Sixty-fours', 'Eights', 'Ones']
  const placeValues = [64, 8, 1]
  const powersOf8 = ['8²', '8¹', '8⁰']
      ...
      <div className="text-center space-y-6">
        ...
        <div className="p-4 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
          ...
          <p className="text-base mt-2" style={{ color: 'var(--text-secondary)' }}>
            Let's calculate it step by step for 345 (octal)
          </p>
        </div>
        ...
        <div className="space-y-4">
          {!isComplete && (
            ...
            >
              {state.currentTier === 0 ? 'Show Positions' : state.currentTier === 1 ? 'Show Place Values' : 'Show Powers of 8'}
            </Button>
          )}
          ...
          {state.currentTier >= 1 && (
            <div className="animate-fade-in p-4 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
              <p className="text-base" style={{ color: 'var(--text-secondary)' }}>
                <strong style={{ color: 'var(--text-primary)' }}>Step 1:</strong> Each position has a name - Sixty-fours, Eights, Ones
              </p>
            </div>
          )}
          ...
          {state.currentTier >= 2 && (
            <div className="animate-fade-in p-4 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
              <p className="text-base" style={{ color: 'var(--text-secondary)' }}>
                <strong style={{ color: 'var(--text-primary)' }}>Step 2:</strong> Each position has a place value - 64, 8, 1
              </p>
            </div>
          )}
          ...
          {state.currentTier >= 3 && (
            <div className="animate-fade-in p-4 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
              <p className="text-base" style={{ color: 'var(--text-secondary)' }}>
                <strong style={{ color: 'var(--text-primary)' }}>Step 3:</strong> Each place value is a power of 8 - 8², 8¹, 8⁰
              </p>
            </div>
          )}
          ...
          {isComplete && (
            <div className="animate-fade-in-up p-6 rounded-xl" style={{ backgroundColor: 'var(--accent-bg)' }}>
              <p className="text-lg font-bold" style={{ color: 'var(--accent-primary)' }}>
                Digit → Position → Power of 8 → Place Value → Contribution
              </p>
            </div>
          )}
          ...
          {isComplete && (
            <div className="animate-fade-in p-6 rounded-xl" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
              <h3 className="text-xl font-bold mb-4" style={{ color: 'var(--text-primary)' }}>
                Complete Calculation: 345 (octal) = 229 (decimal)
              </h3>
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 rounded-lg" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                  <span style={{ color: 'var(--text-secondary)' }}>3 × 64 =</span>
                  <span className="font-bold" style={{ color: 'var(--accent-primary)' }}>192</span>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                  <span style={{ color: 'var(--text-secondary)' }}>4 × 8 =</span>
                  <span className="font-bold" style={{ color: 'var(--accent-primary)' }}>32</span>
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg" style={{ backgroundColor: 'var(--bg-secondary)' }}>
                ...
                <div className="flex items-center justify-between p-3 rounded-lg border-t-2" style={{ backgroundColor: 'var(--accent-bg)', borderColor: 'var(--accent-primary)' }}>
                  ...
                  <span className="font-bold text-xl" style={{ color: 'var(--accent-primary)' }}>229</span>
                </div>
              </div>
            </div>
          )}
