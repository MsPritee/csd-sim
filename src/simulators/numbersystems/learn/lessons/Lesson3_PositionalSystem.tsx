...
export function Lesson3_PositionalSystem({ onComplete }: Lesson3Props) {
  ...
  const number = [5, 5, 5] // 555
  const positionNames = ['Hundreds', 'Tens', 'Ones']
  const positionValues = [500, 50, 5]
    ...
    <div className="space-y-8">
        ...
        {state.showSumAnimation && (
          <div className="animate-fade-in-up space-y-4">
            <div className="flex items-center justify-center gap-2 text-2xl">
              ...
              <span style={{ color: 'var(--text-secondary)' }}>=</span>
              <span className="font-bold" style={{ color: 'var(--accent-primary)' }}>555</span>
            </div>
            ...
            <div className="p-4 rounded-lg" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
              <p className="text-base" style={{ color: 'var(--text-secondary)' }}>
                The same digit (5) has different values depending on its position:
              </p>
              <div className="mt-2 space-y-1">
                <p style={{ color: 'var(--text-primary)' }}>• 5 in ones place = 5</p>
                <p style={{ color: 'var(--text-primary)' }}>• 5 in tens place = 50</p>
                <p style={{ color: 'var(--text-primary)' }}>• 5 in hundreds place = 500</p>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="mt-8 p-6 rounded-xl" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
        ...
        <p className="text-sm mt-2" style={{ color: 'var(--text-muted)' }}>
          💡 Next lesson: We'll learn HOW position value is calculated using powers of 10!
        </p>
      </div>
    </div>
  )
