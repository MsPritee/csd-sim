...
import type { Lesson1State } from '../types/lesson.types'

interface Lesson1Props {
  onComplete?: () => void
}

export function Lesson1_WhatIsHexadecimal({ onComplete }: Lesson1Props) {
  ...
  const everydayHexadecimal = ['A5', 'FF', '1B4', '7FFFFFFF']
  
  const questions = [
    {
      id: 1,
      question: "Why do we use 16 digits?",
    ...
    <div className="space-y-8">
      ...
      <div className="text-center space-y-6">
        <h2 className="text-3xl font-bold" style={{ color: 'var(--text-primary)' }}>
          Meet the Hexadecimal Number System
        </h2>

        {/* Everyday hexadecimal numbers */}
        <div className="space-y-4">
          <p className="text-lg" style={{ color: 'var(--text-secondary)' }}>
            Computers use hexadecimal numbers everywhere:
          </p>
          <div className="flex justify-center gap-6 flex-wrap">
            {everydayHexadecimal.map((num) => (
              <div
                key={num}
        ...
        {state.revealedQuestions.length === 0 && (
          <div className="space-y-4 animate-fade-in">
            <p className="text-xl font-medium" style={{ color: 'var(--text-primary)' }}>
              But have you ever wondered how hexadecimal numbers actually work?
            </p>
            <Button
              variant="primary"
            ...
            {questions.slice(0, state.revealedQuestions.length).map((q, index) => (
              ...
              >
                <div className="space-y-4">
                  ...
                  <div className="p-4 rounded-lg font-mono text-sm whitespace-pre-line text-center" style={{ backgroundColor: 'var(--bg-secondary)', color: 'var(--text-secondary)' }}>
            ...
            {state.allQuestionsRevealed && (
              <div className="animate-fade-in-up p-6 rounded-xl" style={{ backgroundColor: 'var(--accent-bg)' }}>
                <p className="text-lg font-bold" style={{ color: 'var(--accent-primary)' }}>
                  🎯 Great! You've discovered the key questions about hexadecimal numbers.
                </p>
      ...
      <div className="mt-8 p-6 rounded-xl" style={{ backgroundColor: 'var(--bg-tertiary)' }}>
        ...
        <p className="text-base" style={{ color: 'var(--text-secondary)' }}>
          ...
          Hexadecimal is the number system that uses 16 digits (0-9, A-F), perfect for computing!
        </p>
      </div>
    </div>
  )
