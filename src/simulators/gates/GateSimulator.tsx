...
function Card({ title, children }: { title?: string; children: React.ReactNode }) {
  return (
    ...
    >
      {title && (
        <h2 className="mb-3 text-sm font-semibold uppercase tracking-wider" style={{ color: 'var(--accent-primary)' }}>
          {title}
        </h2>
      )}
      {children}
  ...
  return (
    <main className="min-h-screen" style={{ backgroundColor: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
      ...
      <div className="mx-auto max-w-5xl px-4 py-6 space-y-4">
        {/* mode toggle */}
        <div className="flex gap-2" data-testid="simulator-mode">
        ...
        {mode === 'explore' && (
          <>
        {/* gate selector */}
        <div className="flex flex-wrap gap-2" data-testid="gate-selector">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <Card title="Symbol &amp; signals">
            ...
            <div className="mt-4 flex flex-col gap-2">
              ...
              {inputs.map((b, i) => (
                ...
                >
                  <span style={{ color: 'var(--text-secondary)' }}>{`${INPUT_LABELS[i]}`}</span>
                  <span className="font-mono text-lg tabular-nums" style={{ color: b === 1 ? 'var(--accent-primary)' : 'var(--text-muted)' }}>
                    {b}
                  </span>
                </button>
              ))}
              <div
                className="flex items-center justify-between rounded-lg border px-3 py-2 mt-1"
                style={{ borderColor: output === 1 ? 'var(--success-bg)' : 'var(--border-color)', backgroundColor: 'var(--bg-tertiary)' }}
                data-testid="output-badge"
              >
                <span style={{ color: 'var(--text-secondary)' }}>Y</span>
                <span className="font-mono text-xl tabular-nums" style={{ color: output === 1 ? 'var(--success-text)' : 'var(--text-muted)' }}>
                  {output}
                </span>
              </div>
            </div>
          ...
          <Card title="Why this output?">
            <div className="space-y-3">
              <div>
                <div className="font-mono text-xl" data-testid="explanation-what" style={{ color: 'var(--accent-primary)' }}>
                  {explanation.what}
                </div>
                <p className="mt-2 text-sm leading-relaxed" data-testid="explanation-why" style={{ color: 'var(--text-secondary)' }}>
                  {explanation.why}
                </p>
              </div>
              <div
              ...
              <p className="text-sm" style={{ color: 'var(--text-secondary)' }}>
                ...
                <span className="font-mono" data-testid="expression">{def.booleanExpression}</span>
              </p>
            </div>
          </Card>
        </div>
        ...
        <Card title="Laws that hold">
          <div className="flex flex-wrap gap-2">
            {laws.length === 0 && (
              <span className="text-sm" style={{ color: 'var(--text-muted)' }}>None of the cataloged laws apply.</span>
            )}
            {laws.map((law) => (
              <span
                key={law.id}
        ...
        <Card title="Learn the concept">
          ...
          <div className="mt-3 space-y-2 text-sm leading-relaxed" style={{ color: 'var(--text-secondary)' }}>
          ...
          {concept.hints.length > 0 && (
            <div className="mt-3">
              ...
              <ul className="mt-1 list-disc pl-5 text-sm" style={{ color: 'var(--text-secondary)' }}>
                {concept.hints.map((h) => (
                  <li key={h}>{h}</li>
                ))}
              </ul>
